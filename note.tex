...
in a high power, the last step of a chain built over six weeks in 2026: Itty, Rosin,
Carstensen and Reichman \cite{Itty} gave an independent set of size $134753$ in
$C_7^{\bt 10}$, Gao \cite{Gao} recast it as a recursion reaching $C_7^{\bt 200}$, Buys, Polak
and Zuiddam \cite{BPZ} formalised the resulting bounds in Lean, and Tandon \cite{Tandon}
strengthened the recursion. A single sixth-power set would have to reach $1198$ to match that
record, and $1129^{1/6} = 3.22687635\ldots$.
...
\section*{Data, code and assistance}

The two certificates and both verifiers accompany this note as ancillary files and need only
a stock Node.js 20 or Python 3. The same files, together with the search code, are at
\url{https://github.com/Ratatosk32/c7-independent-sets} and are archived at
\href{https://doi.org/10.5281/zenodo.22643652}{doi:10.5281/zenodo.22643652}.

The local search, the layer moves of Section~5 and both verifiers were carried out with the
assistance of a large language model. The result does not rest on that. What is claimed here is
a single list of $1129$ vertices, and either verifier settles it outright by the naive all-pairs
test, which is slow but short enough to audit by reading; the two share no code and have no
dependencies. A certificate is true or false on its own terms, whatever produced it.
...
\bibitem{BPZ}
P.~Buys, S.~Polak and J.~Zuiddam,
\emph{Lean-verified lower bounds for the Shannon capacity of odd cycles},
arXiv:2607.29681.

\bibitem{Gao}
Y.~Gao,
\emph{A recursive construction improving the lower bound on the Shannon capacity of $C_7$},
arXiv:2607.27869.
